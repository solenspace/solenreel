...
- **Hosting (functions)**: Supabase Edge Functions are deployed via `pnpx supabase functions deploy <name>` against the linked project. Service-role key + OpenRouter key + TMDB key are set in the Supabase dashboard's function env (not in repo, not in any committed file).
...
- **README**: replaces `NETFLIX-CLONE-README.md` (preserved for attribution) with a fresh `README.md` for reel: what it is, who it's for, how to run, attribution, license note.
...
5. Add Playwright as a devDep; create `tests/smoke/reel.spec.ts`. Smoke runs locally via `pnpm test:smoke -- --base-url <url>`. Document in README.
6. Deploy:
...
7. `grep -ri "netflix" src/` returns 0 (acceptable hits only in `NETFLIX-CLONE-README.md` and migration comments).
