    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-4xl bg-bg-elevated rounded-lg overflow-hidden shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-20 w-9 h-9 rounded-full bg-bg-elevated/80 flex items-center justify-center text-white hover:bg-bg-elevated transition-colors text-xl"
          >
            ✕
          </button>
          ...
          <div className="relative w-full aspect-video bg-bg">
            ...
            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-bg-elevated to-transparent" />
                  ...
                  {cast.map((person) => (
                    <div key={person.id} className="flex-shrink-0 w-20 text-center">
                      {person.profile_path ? (
                        ...
                        <div className="w-16 h-16 rounded-full bg-bg-elevated mx-auto mb-1 flex items-center justify-center text-gray-500 text-xs">
                          N/A
                        </div>
                      )}
            ...
            {similar.length > 0 && (
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {similar.map((item) => (
                    <div key={item.id} className="bg-bg-elevated rounded-md overflow-hidden hover:ring-1 hover:ring-white/20 transition-all">
                      {item.backdrop_path ? (
                        <div
                        ...
                        <div className="w-full aspect-video bg-bg flex items-center justify-center text-gray-600 text-sm">
                          No Image
                        </div>
                      )}
                      <div className="p-3">
